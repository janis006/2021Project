...
test.dy.ts <- ts(data.frame(test.dy), frequency = 12, start = c(2020,9))

# start creating ts object
DynData.sel_ts<-ts(data.frame(train.dy), frequency = 12, start = c(2018,9))

# prepare the whole ts
declaration.ts<-ts(data.frame(DynData.sel[,1]), frequency = 12, start = c(2018,9))

################################################################################
# dynamic regression model
# one variable xreg: death
(dyn.fit <- auto.arima(DynData.sel_ts[,"declaration"], xreg=DynData.sel_ts[,"death"]))
checkresiduals(dyn.fit)

# forcast
plot(cbind(declaration.ts, forecast(dyn.fit, xreg=rep(mean(DynData.sel_ts[,"death"]),6))$mean),      
     plot.type = "single", ylim=c(-200000,2000000),
     ylab = "UC Declaration", xaxt='n', main="Forecasts from Regression with ARIMA(0,0,0) errors For the Declaration")
lines(declaration.ts)
...
(dyn.fit1 <- auto.arima(DynData.sel_ts[,"declaration"], xreg=DynData.sel_ts[,"GDP"]))
checkresiduals(dyn.fit1)

# forcast
plot(cbind(declaration.ts, forecast(dyn.fit1, xreg=rep(mean(DynData.sel_ts[,"GDP"]),6))$mean), 
     plot.type = "single", ylim=c(-200000,1400000),
     ylab = "UC Declaration", xaxt='n', main="Forecasts from Regression with ARIMA(0,0,1) errors For Declaration")
lines(declaration.ts)
